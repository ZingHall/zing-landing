export const metadata = {
  title: 'CLI',
  description:
    'Learn about the zing CLI tool for paid semantic search and chunk retrieval on the Zing platform.',
}

# CLI

CLI tool for paid semantic search and chunk retrieval on the Zing platform. Executes USDC micro-payments on Sui and queries the Search API. {{ className: 'lead' }}
...
<Row>
  <Col>
    ...
    Start an MCP server on stdio for AI agent integration. Exposes `zing_search`, `zing_chunks`, and `zing_expand_chunks` as MCP tools. See the [MCP Server](/mcp-server) page for full documentation with client setup and tool reference.
...
- [MCP Server](/mcp-server) - AI agent integration with MCP tools
