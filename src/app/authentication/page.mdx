...
When making API requests to the Zing API, authentication uses your Sui wallet signature:

```bash
# API calls are authenticated with your Sui wallet signature
curl https://api.prod.zing.you/articles \
  -H "x-signature: {base64_signature}" \
  -H "x-message-bytes: {base64_message}"

# The server verifies the signature and extracts your wallet address
```

For the Search API, paid access uses a signed `ApiAccessMessage`:
...
curl -X POST https://search.zing.services/chunks \
  ...
  -d '{
    ...
    "bytes": "{base64_bcs_message}",
    "transaction_digest": "{sui_tx_hash}"
  }'
```

For internal app access (free endpoints), use the app key:

```bash
curl "https://search.zing.services/search?q=query" \
